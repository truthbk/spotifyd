...
//
// should this be threaded? I think not, player will round robin sessions.
class SpotifySession {
	public:
		SpotifySession( sp_session * sess ) 
			:m_sess(sess)
			 ,m_notify_do(0)
			 ,m_playback_done(1)
...
class SpotifyHandler : virtual public SpotifyIf {
	public:
		SpotifyHandler(const uint8_t *appkey, const size_t appkey_size) {
			...
			if (m_handler_ptr) {
				return m_handler_ptr; // Singleton stuff not done like this...
			}
		...
		void initiateSession(SpotifyCredential& _return, const SpotifyCredential& cred) {
			...
			shared_ptr< sp_session > sess = getSession(cred);
			if(!sess) {
				sess = shared_ptr< sp_session>(new sp_session()));
			...
			shared_ptr<sp_playlistcontainer> pc(sp_session_playlistcontainer(sess));
			sessions.insert(cred, pc);
		}
	...
	private:
		...
		shared_ptr<SpotifySession> getSession(SpotifyCredential& cred) {
			...
			{
				return NULL; //or something similar with BOOST
			}
		...
		shared_ptr<sp_playlistcontainer> getPlaylistContainer(SpotifyCredential& cred) {
