...
typedef std::set<class SpotifyTrack>  SpotifyPlaylist;
...
typedef struct _SpotifyCredential__isset {
  _SpotifyCredential__isset() : _uid(false) {}
  bool _uid;
...
class SpotifyCredential {
 public:
  ...
  SpotifyCredential() : _username(""), _passwd(""), _uid("") {
  }
  ...
  bool operator == (const SpotifyCredential & rhs) const
  {
    ...
    if (!(_passwd == rhs._passwd))
      return false;
    if (__isset._uid != rhs.__isset._uid)
      return false;
    else if (__isset._uid && !(_uid == rhs._uid))
      return false;
    return true;
  }
  bool operator != (const SpotifyCredential &rhs) const {
